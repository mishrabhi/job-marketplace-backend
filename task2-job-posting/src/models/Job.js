...
export default class Job {
  ...
  static async listByCompanyId(companyId) {
    const result = await query(
      `SELECT ${JOB_SELECT} FROM jobs WHERE company_id = $1 ORDER BY created_at DESC`,
      [companyId]
    );
    return result.rows;
  }

  static async listPublished(limit = 50, offset = 0) {
    const result = await query(
      `SELECT ${JOB_SELECT} FROM jobs WHERE status = 'published' ORDER BY created_at DESC LIMIT $1 OFFSET $2`,
      [limit, offset]
    );
    return result.rows;
  }
