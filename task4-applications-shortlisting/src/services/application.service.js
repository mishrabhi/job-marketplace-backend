...
export default class ApplicationService {
  ...
  static async getStudentApplications(studentId, limit = 50, offset = 0) {
    const studentResult = await query(`SELECT id FROM students WHERE id = $1`, [studentId]);
    if (studentResult.rows.length === 0) {
      const error = new Error('Student not found');
      error.status = 404;
    ...
    const applications = await Application.findByStudent(studentId, limit, offset);
    return applications;
  }
  ...
  static async withdrawApplication(applicationId) {
    ...
    if (application.status !== 'applied') {
      const error = new Error(`Cannot withdraw application with status ${application.status}`);
      error.status = 400;
      error.code = 'INVALID_APPLICATION_STATUS';
      throw error;
    }

    const withdrawn = await Application.withdraw(applicationId);
    return withdrawn;
  }
}
